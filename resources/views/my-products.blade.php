...
@section('content')
    <div id="my_products_row" class="row">
        <!-- نوار کناری دسته‌بندی‌ها -->
        <div class="col-md-3">
            <button class="btn btn-secondary category-toggle-btn" id="toggleCategories">
                افزودن دسته بندی
            </button>
            <div class="sidebar">
                ...
                @foreach($products as $product)
                    <div class="custom-product-card">
                        ...
                        <div class="custom-product-body">
                            ...
                            @if($product->discount > 0)
                                <div
                                    class="custom-product-old-price">{{ toPersianNumber(number_format($product->price)) }}
                                    تومان
                                </div>
                                <div
                                    class="custom-product-price">{{ toPersianNumber(number_format($product->price - ($product->price * $product->discount / 100))) }}
                                    تومان
                                </div>
                            @else
                                <div class="custom-product-price">{{ toPersianNumber(number_format($product->price)) }}
                                    تومان
                                </div>
                            @endif
